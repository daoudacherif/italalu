...
if (isset($_POST['submit'])) {
    ...
    if ($amount <= 0) {
        echo "<script>alert('Invalid amount. Must be > 0');</script>";
    } else {
        ...
        if (mysqli_query($con, $sqlInsert)) {
            echo "<script>alert('Transaction recorded!');</script>";
        } else {
...
<html lang="en">
<head>
    <title>Inventory Management | Cash Transactions</title>
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb">
      ...
      <a href="transact.php" class="current">Cash Transactions</a>
    </div>
    <h1>Cash Transactions (Daily Sale + Manual Deposit/Withdrawal)</h1>
  </div>
    ...
    <div class="row-fluid">
      <div class="span12">
        <div style="border: 1px solid #ccc; padding: 15px; margin-bottom: 20px;">
          <h4>Current Balance: <?php echo number_format($currentBalance, 2); ?></h4>
          <p>Today IN: <?php echo number_format($todayIn, 2); ?>,
             Today OUT: <?php echo number_format($todayOut, 2); ?>,
             Net: <?php echo number_format($todayNet, 2); ?></p>
          <p>Today's Cart Sale: <?php echo number_format($todysale, 2); ?><?php
             if ($alreadyInserted) {
                 echo " (already added to cash)";
             }
          ?></p>
        </div>
      </div>
